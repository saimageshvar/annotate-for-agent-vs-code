"use strict";
(() => {
  // src/views/webview/editor/main.ts
  var vscode = acquireVsCodeApi();
  var mode = "empty";
  var annotation = {};
  var categoryMeta = {};
  var defaultCategory = "Suggestion";
  window.addEventListener("message", (event) => {
    ...
    if (msg.type === "init-create") {
      ...
      annotation.context = msg.payload.context;
      render();
      ...
      insertFileRef(msg.path, msg.cursorAt);
    }
  });
  function insertFileRef(path, cursorAt) {
    const ta = document.getElementById("comment");
    if (!ta)
      return;
    ta.focus();
  ...
  function renderPath(filePath, range) {
  ...
  function renderView(app) {
    ...
    <div class="view-meta">
      ...
      <div class="small-meta">
        ...
        ${a.updatedAt !== a.createdAt ? `<span> \xB7 Updated ${relativeTime(a.updatedAt)}</span>` : ""}
      </div>
    </div>
  ...
  var SUGGESTION_CATEGORIES = /* @__PURE__ */ new Set(["Bug", "Refactor", "Nit", "Suggestion"]);
  ...
  var categoryGlobalKeyHandler = null;
  function wireCategoryPills(cats) {
    const pills = Array.from(document.querySelectorAll(".cat-pill"));
    if (pills.length === 0)
      return;
    const selectIndex = (i, focus = true) => {
      const c = cats[i];
      if (!c)
        return;
      ...
      if (hintEl)
        hintEl.textContent = categoryMeta[c]?.userHint ?? "";
      const insertBtn = document.getElementById("insertSuggest");
      if (insertBtn) {
        if (SUGGESTION_CATEGORIES.has(c))
    ...
    if (categoryGlobalKeyHandler)
      document.removeEventListener("keydown", categoryGlobalKeyHandler, true);
    categoryGlobalKeyHandler = (e) => {
    ...
    pills.forEach((pill, idx) => {
      ...
      pill.addEventListener("keydown", (e) => {
        if (e.key === "ArrowRight" || e.key === "ArrowDown") {
          e.preventDefault();
          selectIndex((idx + 1) % cats.length);
        } else if (e.key === "ArrowLeft" || e.key === "ArrowUp") {
          e.preventDefault();
          selectIndex((idx - 1 + cats.length) % cats.length);
        } else if (e.key === "Home") {
          e.preventDefault();
          selectIndex(0);
        } else if (e.key === "End") {
          e.preventDefault();
          selectIndex(cats.length - 1);
        } else if (e.key.length === 1) {
          const ch = e.key.toLowerCase();
          if (/^[1-9]$/.test(ch)) {
            const target2 = parseInt(ch, 10) - 1;
            if (target2 < cats.length) {
              e.preventDefault();
              selectIndex(target2);
            }
            return;
  ...
  function insertSuggestionBlock(ta) {
    const ctx = annotation.context;
    const snippet = ctx?.snippet ?? ctx?.after ?? "";
